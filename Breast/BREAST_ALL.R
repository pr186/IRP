...
OUTPUT <- "/home/pr186/Documents/IRP/FINAL/BREAST/ALL"
...
# TRIPLE NEGATIVE
setwd(INPUT)
CANCER_01_matrix <- ReadMtx(mtx = "GSM4909282_TN-MH0135-matrix.mtx.gz", 
                            features = "features.tsv", 
                            cells = "GSM4909282_TN-MH0135-barcodes.tsv.gz")
CANCER_01_matrix

CANCER_01_object <- CreateSeuratObject(counts = CANCER_01_matrix, project = "TN_TUMOR", min.cells = 3, min.features = 200)
CANCER_01_object$Type <- "TN_TUMOR"
...
setwd(INPUT)
CANCER_02_matrix <- ReadMtx(mtx = "GSM4909315_ER-MH0167-T-matrix.mtx.gz", 
                            features = "features.tsv", 
                            cells = "GSM4909315_ER-MH0167-T-barcodes.tsv.gz")
CANCER_02_matrix

CANCER_02_object <- CreateSeuratObject(counts = CANCER_02_matrix, project = "ER+_TUMOR", min.cells = 3, min.features = 200)
CANCER_02_object$Type <- "ER+_TUMOR"

setwd(OUTPUT)
png(file = "GENE_COUNT_3_BF_FILTER.png")
VlnPlot(CANCER_02_object, features = "nCount_RNA")
dev.off()

CANCER_02_object <- subset(CANCER_02_object, subset = nCount_RNA < 6500)

CANCER_02_object <- NormalizeData(CANCER_02_object)
CANCER_02_object <- FindVariableFeatures(CANCER_02_object, selection.method = "vst", nfeatures = 2000)
CANCER_02_object


top10_TN_1 <- head(VariableFeatures(CANCER_02_object), 10)

plot5 <- VariableFeaturePlot(CANCER_02_object)
plot6 <- LabelPoints(plot = plot5, points = top10_TN_1, repel = TRUE)
...
setwd(INPUT)
CANCER_03_matrix <- ReadMtx(mtx = "GSM4909294_HER2-MH0176-matrix.mtx.gz", 
                            features = "features.tsv", 
                            cells = "GSM4909294_HER2-MH0176-barcodes.tsv.gz")
CANCER_03_matrix

CANCER_03_object <- CreateSeuratObject(counts = CANCER_03_matrix, project = "HER+_TUMOR", min.cells = 3, min.features = 200)
CANCER_03_object$Type <- "HER2+_TUMOR"

setwd(OUTPUT)
png(file = "GENE_COUNT_3_BF_FILTER.png")
VlnPlot(CANCER_03_object, features = "nCount_RNA")
dev.off()

CANCER_03_object <- subset(CANCER_03_object, subset = nCount_RNA < 6500)

CANCER_03_object <- NormalizeData(CANCER_03_object)
CANCER_03_object <- FindVariableFeatures(CANCER_03_object, selection.method = "vst", nfeatures = 2000)
CANCER_03_object


top10_TN_1 <- head(VariableFeatures(CANCER_03_object), 10)

plot5 <- VariableFeaturePlot(CANCER_03_object)
plot6 <- LabelPoints(plot = plot5, points = top10_TN_1, repel = TRUE)
...
combined.anchors <- FindIntegrationAnchors(object.list = list(CANCER_01_object,
                                                              CANCER_02_object,
                                                              CANCER_03_object), dims = 1:20)
...
dittoBarPlot(combined, "Type", group.by = "seurat_clusters", color.panel = c("#F8766D", "#0CB702", "#00BFC4"))
dev.off()
...
TARGET <- c("SIX1", "SIX2", "PAX6", "DACH1", "DACH2", "EYA1", "EYA2")
...
DotPlot(combined, features = rev(TARGET), cols = c("blue", "red", "green"), dot.scale = 10, group.by = "Type")
dev.off()
...
DotPlot(combined, features = rev(TARGET), cols = c("blue", "red", "green"), dot.scale = 10, split.by = "Type")
dev.off()

png(file = "DOT_TARGET_SMALL.png", width = 700, height = 900)
DotPlot(combined, features = rev(TARGET), cols = c("blue", "red", "green"), dot.scale = 10, split.by = "Type")
dev.off()
...
DotPlot(combined_annotated, features = rev(TARGET), cols = c("blue", "red", "green"), dot.scale = 10, split.by = "Type")
dev.off()
...
p12 <- DimPlot(combined_annotated, reduction = "tsne", label = FALSE, pt.size = 0.5, label.size = 6, split.by = "Type") 
plot_grid(p12)

png(file = "ALL_SEP_ANN.png", width = 1200, height = 500)
plot_grid(p12)
dev.off()

p13 <- DimPlot(combined_annotated, reduction = "umap", label = FALSE, pt.size = 0.5, label.size = 6, split.by = "Type") 
plot_grid(p13)

png(file = "ALL_SEP_ANN_UMAP.png", width = 1200, height = 500)
plot_grid(p13)
dev.off()
...
DotPlot(combined_annotated, features = rev(TARGET2), cols = c("blue", "red", "green"), dot.scale = 10, group.by = "Type")
dev.off()
